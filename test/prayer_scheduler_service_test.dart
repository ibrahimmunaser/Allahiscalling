...
void main() {
  ...
  group('scheduling next 7 days', () {
    ...
    test(
      'follow-ups fire 20 minutes after the prayer, within the window',
      () async {
        final scheduler = buildScheduler();
        final reminders = await scheduler.rescheduleAll(settings);
        final prayerTimes = {
        ...
        for (final f in reminders.where(
          (r) => r.kind == ReminderKind.followUp,
        )) {
          ...
          final matchesAPrayer = prayerTimes.contains((
            f.prayer,
            f.scheduledAtMillis - delay,
          ));
          final isCurrentWindow =
              f.scheduledAtMillis - delay <= fixedNow.millisecondsSinceEpoch;
    ...
    test('persists scheduled IDs and bookkeeping', () async {
      ...
      expect(
        persisted.where((r) => r.kind == ReminderKind.prayer),
        hasLength(34),
      );
      expect(
        persisted.where((r) => r.kind == ReminderKind.followUp),
        hasLength(33),
      );
  ...
  group('skipping past prayer times', () {
    test('skips everything before "now" late in the day', () async {
      ...
      final first = reminders.reduce(
        (a, b) => a.scheduledAtMillis < b.scheduledAtMillis ? a : b,
      );
  ...
  group('duplicate notification prevention', () {
    test(
      'rescheduling cancels old reminders before scheduling new ones',
      () async {
        final scheduler = buildScheduler();
        ...
        expect(
          second.map((r) => r.notificationId).toSet(),
          first.map((r) => r.notificationId).toSet(),
        );
      },
    );
  ...
  group('notifications disabled or unconfigured', () {
    ...
    test('schedules nothing without a location', () async {
      final scheduler = buildScheduler();
      final reminders = await scheduler.rescheduleAll(
  ...
  group('iOS scheduling policy (pending-notification cap)', () {
    test('pending count never exceeds 60 with the iOS policy', () async {
      final scheduler = buildScheduler(policy: const SchedulingPolicy.ios());
      final reminders = await scheduler.rescheduleAll(settings);
    ...
    test(
      'baseline primary reminders are never trimmed to fit the cap',
      () async {
        // Android baseline: how many primaries a full 7-day window contains.
        final androidScheduler = buildScheduler(
          policy: const SchedulingPolicy.android(),
        );
        final androidReminders = await androidScheduler.rescheduleAll(settings);
        final baselinePrimaries =
            androidReminders.where((r) => r.kind == ReminderKind.prayer).length;
        ...
        final iosScheduler = buildScheduler(
          policy: const SchedulingPolicy.ios(),
        );
    ...
    test(
      'remaining capacity is filled with extra primary days (dynamic)',
      () async {
        final scheduler = buildScheduler(policy: const SchedulingPolicy.ios());
        final reminders = await scheduler.rescheduleAll(settings);
        final primaries =
        ...
        expect(primaries.length + followUps.length + 1, lessThanOrEqualTo(60));
        // Coverage extends beyond 8 days.
        final last = primaries.reduce(
          (a, b) => a.scheduledAtMillis > b.scheduledAtMillis ? a : b,
        );
        expect(
    ...
    test('the final scheduled primary prayer still exists', () async {
      ...
      final reminders = await scheduler.rescheduleAll(settings);
      final primaries =
          reminders.where((r) => r.kind == ReminderKind.prayer).toList()..sort(
            (a, b) => a.scheduledAtMillis.compareTo(b.scheduledAtMillis),
          );
      ...
      final refresh = reminders.singleWhere(
        (r) => r.kind == ReminderKind.refresh,
      );
    ...
    test('follow-ups are limited to the 48-hour horizon', () async {
      ...
      for (final f in followUps) {
        expect(
          f.scheduledAt.isAfter(horizon),
          isFalse,
          reason:
              'follow-up for ${f.prayer} at ${f.scheduledAt} is '
    ...
    test('a tight cap trims only follow-ups, furthest-out first', () async {
      ...
      expect(
        maxKept,
        lessThan(
          fixedNow.add(const Duration(hours: 48)).millisecondsSinceEpoch,
        ),
      );
    });

    test(
      'carried snoozes reserve slots that would otherwise hold primaries',
      () async {
        final scheduler = buildScheduler(policy: const SchedulingPolicy.ios());
    ...
    test(
      'degenerate cap smaller than the baseline still never exceeds it',
      () async {
        ...
        expect(
          primaries.first.scheduledAt.isBefore(primaries.last.scheduledAt),
          isTrue,
        );
  ...
  group('rolling-window replenishment (seven-day cliff)', () {
    test(
      'a refresh safety notification is scheduled after the last primary',
      () async {
        final scheduler = buildScheduler();
        final reminders = await scheduler.rescheduleAll(settings);

        final refresh =
            reminders.where((r) => r.kind == ReminderKind.refresh).toList();
        expect(refresh, hasLength(1));
        expect(
          refresh.single.notificationId,
    ...
    test('replenishment keeps the full primary horizon scheduled', () async {
      ...
      expect(
        lastPrimary.scheduledAt.isAfter(laterNow.add(const Duration(days: 6))),
        isTrue,
      );
    });
  ...
  group('user-requested snoozes survive rescheduling', () {
    test('future snoozes are kept, expired ones are dropped', () async {
      ...
      final expiredSnooze = ScheduledReminder(
        ...
        scheduledAtMillis:
            fixedNow.subtract(const Duration(hours: 1)).millisecondsSinceEpoch,
        kind: ReminderKind.snooze,
      );
  ...
  group('recalculation triggers', () {
    ...
    test('significant location change is detected', () async {
      ...
      expect(scheduler.hasLocationChangedSignificantly(40.72, -74.01), isFalse);
