...
void main() {
  ...
  group('four distinct response states', () {
    test('answered is persisted and readable', () async {
      ...
      await repository.savePrayerResponse(
          day, SalahPrayer.dhuhr, PrayerResponseState.answered, at);

      expect(repository.prayerResponse(day, SalahPrayer.dhuhr),
    ...
    test('declined is persisted and is not answered', () async {
      await repository.savePrayerResponse(day, SalahPrayer.asr,
          PrayerResponseState.declined, DateTime(2026, 7, 4, 17, 2));

      expect(repository.prayerResponse(day, SalahPrayer.asr),
    ...
    test('dismissed is persisted and is neither answered nor declined',
        () async {
      await repository.savePrayerResponse(day, SalahPrayer.maghrib,
          PrayerResponseState.dismissed, DateTime(2026, 7, 4, 21, 40));

      expect(repository.prayerResponse(day, SalahPrayer.maghrib),
          PrayerResponseState.dismissed);
      expect(repository.prayerAnsweredAt(day, SalahPrayer.maghrib), isNull);
    });

    test('marked complete is tracked independently of the response',
        () async {
  ...
  group('state transitions', () {
    test('latest response wins: dismissed then answered ends answered',
        () async {
      await repository.savePrayerResponse(day, SalahPrayer.dhuhr,
          PrayerResponseState.dismissed, DateTime(2026, 7, 4, 13, 7));
      await repository.savePrayerResponse(day, SalahPrayer.dhuhr,
          PrayerResponseState.answered, DateTime(2026, 7, 4, 13, 30));

      expect(repository.prayerResponse(day, SalahPrayer.dhuhr),
          PrayerResponseState.answered);
      expect(repository.prayerAnsweredAt(day, SalahPrayer.dhuhr),
          DateTime(2026, 7, 4, 13, 30));
    });

    test('responses are per-prayer and per-day', () async {
      await repository.savePrayerResponse(day, SalahPrayer.dhuhr,
          PrayerResponseState.answered, DateTime(2026, 7, 4, 13, 6));
